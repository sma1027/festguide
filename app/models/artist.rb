...
class Artist < ActiveRecord::Base
  ...
  has_one :instagram_account, -> { includes :instagram_posts }, :dependent => :destroy
  accepts_nested_attributes_for :instagram_account
  has_many :instagram_posts, :through => :instagram_account

  has_one :twitter_account, -> { includes :twitter_tweets }, :dependent => :destroy
  accepts_nested_attributes_for :twitter_account
  has_many :twitter_tweets, :through => :twitter_account

  has_one :youtube_account, -> { includes :youtube_videos }, :dependent => :destroy
